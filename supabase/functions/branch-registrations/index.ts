...
interface BranchRegistrationsRequest {
...
serve(async (req) => {
  ...
  try {
    const { action, branchId, registrationId, memberId, newStatus, notes }: BranchRegistrationsRequest = await req.json()

    console.log('Branch Registrations Request:', { action, branchId, registrationId, memberId, newStatus })
    ...
    switch (action) {
      case 'list':
        // Get all registration requests for this branch
      ...
      case 'update_status':
        ...
        if (newStatus === 'approved') {
          updateData.manager_review_date = new Date().toISOString()
          updateData.manager_notes = notes || 'تمت الموافقة على الطلب'
        ...
        const workflowStepMap: Record<string, string> = {
          ...
          'under_manager_review': 'manager_review_started',
          'approved': 'approved',
          'rejected': 'rejected',
      ...
      case 'get':
        ...
        const { data: documents, error: documentsError } = await supabase
          ...
          .select(`
            ...
            verification_status, is_required, verified_at
          `)
    ...
    console.error('Branch Registrations Error:', error)
    return new Response(
      JSON.stringify({ 
        success: false, 
